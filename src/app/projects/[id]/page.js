        ...
        <div className="bg-white rounded-3xl p-12">
          <div className="grid grid-cols-2 gap-12 items-start mb-8">
            ...
            <div>
              ...
              <p className="text-[#3f3737] text-[25px] font-sans font-medium leading-relaxed mb-12">
                {heroDescription}
              </p>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-8 mb-8">
                {detailEntries.map((entry) => (
                  <div key={entry.label}>
                    <p className="text-[25px] text-[#4A7C59] font-heading font-bold mb-2 leading-tight">{entry.label}</p>
                    {Array.isArray(entry.value) ? (
                      entry.value.map((item) => (
                        <p
                          key={`${entry.label}-${item}`}
                          className="text-[25px] text-[#3f3737] font-sans font-medium leading-relaxed"
                        >
                          {item}
                        </p>
                      ))
                    ) : (
                      <p className="text-[25px] text-[#3f3737] font-sans font-medium leading-relaxed">
                        {entry.value}
                      </p>
                    )}
                  </div>
      ...
      <div className="container max-w-[1500px] mx-auto px-8 pb-12 pt-12">
        <div className="grid grid-cols-2 gap-12 items-start">
          ...
          <div className="pt-[-10px]">
            <h2 className="font-heading text-[50px] font-semibold text-black leading-tight ml-[20px]">
              Table Of Contents
            </h2>
          </div>
            ...
            <div className="space-y-0">
              {(caseStudy?.tableOfContents ?? []).map((item, index) => {
                ...
                return (
                  ...
                  >
                    <span className="text-[25px] text-[#4A7C59] font-heading font-medium">
                      {number}
                    </span>
                    <a
                      href={`#${item.id}`}
                      className="text-[25px] text-[#3f3737] font-medium hover:text-black"
                    >
                      {item.title}
                    </a>
                  </div>
      ...
      <div className="py-8">
        <div className="container max-w-[1500px] mx-auto px-8">
          <div className="flex items-center justify-between">
            <p className="text-sm text-[#3f3737] font-medium flex items-center gap-2">
              made with love & care
            ...
            <div className="flex items-center gap-4">
              <a href="https://www.linkedin.com/in/irishclairecatungal" target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition" aria-label="LinkedIn profile">
                <svg className="w-5 h-5 text-black" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
              </a>
              <a href="https://github.com/ICClaire" target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition" aria-label="GitHub profile">
                <svg className="w-5 h-5 text-black" fill="currentColor" viewBox="0 0 24 24">
              ...
              <a href="mailto:irishclaireparayno@gmail.com" className="hover:opacity-70 transition" aria-label="Email Irish Claire">
